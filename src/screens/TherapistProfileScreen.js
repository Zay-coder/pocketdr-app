...
function TherapistProfileScreen({navigation, route}) {
    ...
    const getUpvote = async ()=> {
        const response = await axios.get('http://192.168.1.102:8000/api/upvote/'+id);
    ...
    const getDownvote = async ()=> {
        const response = await axios.get('http://192.168.1.102:8000/api/downvote/'+id);
        console.log('hasdy')
    ...
    return therapists ? (
        <View style={styles.container}>
                    <View key={therapists.id}>
                        <View style={{flexDirection: 'row', borderWidth: 1, borderColor: '#fff'}}>
                            ...
                            <View style={{marginBottom: 10, marginTop: 10, marginLeft: 10}}>
                                ...
                                <Text style={{fontFamily: 'JosefinSans_400Regular', fontSize: 15}}> {therapists.upvotes}
                                ...
                                <Text style={{fontFamily: 'JosefinSans_400Regular', fontSize: 15}}> {therapists.downvotes}
            ...
            <View style={{alignItems:'center'}}>
                <TouchableOpacity onPress={() => navigation.navigate('availabledates',{therapist:therapists})} style={styles.button}>
